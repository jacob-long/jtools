...
```{r}
fitc <- lm(mpg ~ hp*wt, data=mtcars)
probe_interaction(fitc, pred = hp, modx = wt, cond.int = TRUE, interval = TRUE)
```
...
It returns an object with each of the two function's return objects:

```{r}
out <- probe_interaction(fitc, pred = hp, modx = wt, cond.int = TRUE, interval = TRUE)
names(out)
```
## 3-way interactions
