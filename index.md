...
[`johnson_neyman()`](reference/interactions_deprecated.md)) have been
...
[`summ()`](reference/summ.md) is a replacement for
[`summary()`](https://rdrr.io/r/base/summary.html) that provides the
user several options for formatting regression summaries. It supports
`glm`, `svyglm`, and `merMod` objects as input as well. It supports
calculation and reporting of robust standard errors via the `sandwich`
package.
...
Of course, [`summ()`](reference/summ.md) like
[`summary()`](https://rdrr.io/r/base/summary.html) is best-suited for
interactive use. When it comes to sharing results with others, you want
sharper output and probably graphics. `jtools` has some options for
that, too.

### LaTeX-, Word-, and RMarkdown-friendly regression summary tables (`export_summs()`)

For tabular output, [`export_summs()`](reference/export_summs.md) is an
interface to the `huxtable` package’s `huxreg()` function that preserves
the niceties of [`summ()`](reference/summ.md), particularly its
facilities for robust standard errors and standardization. It also
...
[`export_summs()`](reference/export_summs.md) and the chunk option
...
with [`plot_summs()`](reference/plot_summs.md) (or the closely related
[`plot_coefs()`](reference/plot_summs.md)). Like with
[`export_summs()`](reference/export_summs.md), you can still get your
scaled models and robust standard errors.
...
[`plot_coefs()`](reference/plot_summs.md) works much the same way, but
without support for [`summ()`](reference/summ.md) arguments like
`robust` and `scale`. This enables a wider range of models that have
support from the `broom` package but not for
...
[`effect_plot()`](reference/effect_plot.md) lets you plot predictions
across values of a predictor variable alongside the observed data.
...
- [`gscale()`](reference/gscale.md): Scale and/or mean-center data,
  including `svydesign` objects
- [`scale_mod()`](reference/scale_mod.md) and
  [`center_mod()`](reference/center_mod.md): Re-fit models with scaled
  and/or mean-centered data
- [`wgttest()`](reference/wgttest.md) and
  [`pf_sv_test()`](reference/pf_sv_test.md), which are combined in
  [`weights_tests()`](reference/weights_tests.md): Test the ignorability
  of sample weights in regression models
- [`svycor()`](reference/svycor.md): Generate correlation matrices from
  `svydesign` objects
- [`theme_apa()`](reference/theme_apa.md): A mostly APA-compliant
  `ggplot2` theme
- [`theme_nice()`](reference/theme_nice.md): A nice `ggplot2` theme
- [`add_gridlines()`](reference/gridlines.md) and
  [`drop_gridlines()`](reference/gridlines.md): `ggplot2` theme-changing
  convenience functions
- [`make_predictions()`](reference/make_predictions.md): an easy way to
  generate hypothetical predicted data from your regression model for
