...
dsa <-
  adjustedcranlogs::adj_cran_downloads(packages = "panelr",
                                       from = "2016-02-28", to = Sys.Date())
dsa <- dsa[which(dsa$total_downloads > 0)[1]:nrow(dsa),]
ds <-
  cranlogs::cran_downloads(packages = "panelr",
                                       from = "2017-02-28", to = Sys.Date())
library(ggplot2)
dsa$avg_7 <- stats::filter(dsa$count, rep(1, 7), sides = 1)/7
dsa$adj_avg_7 <- stats::filter(dsa$adjusted_downloads, rep(1, 7), sides = 1)/7
...
ggplot(data = dst) +
  ...
  jtools::theme_apa(legend.pos = "topleft", legend.use.title = TRUE,
                    ...
                    legend.font.size = 10) +
  theme(legend.title = element_text(size = 11)) +
  ggtitle("panelr daily download trends") +
  labs(caption = "Dashed vertical lines show dates of updates")
